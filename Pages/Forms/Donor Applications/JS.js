...
function validateCurrentSection() {
  ...
  // تحقق من اختيار مجال واحد على الأقل في صفحة الاهتمامات
  if (currentSectionElement.id === "interestsSection") {
    const interests = currentSectionElement.querySelectorAll('input[name="interests[]"]:checked');
  ...
  if (isFormValid) {
    ...
    setTimeout(() => {
      ...
      Swal.fire({
        ...
        text: 'شكراً لك على تقديم بياناتك. سنتواصل معك قريباً.',
  ...
  inputs.forEach(input => {
    if (input.name) {
      ...
      if (input.type === "checkbox") {
        const checked = section.querySelectorAll(`input[name="${input.name}"]:checked`);
        formData[sectionName][input.name] = Array.from(checked).map(c => c.value);
      } else {
